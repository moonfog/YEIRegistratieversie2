...
def overview():
	if len(request.args)!= 0:
		...
		form = SQLFORM.grid(talks,fields=fields,searchable=False,deletable=False,editable=False,details=False,paginate=10,create=False,csv=False,orderby=[~db.talk.date_talk],
			links = [lambda row:A(T('Details'),_href=URL("talk","details",args=[row.id]))], user_signature=False)
		form[1].append(TAG.INPUT(_value=T('Add Talk'),_type="button",_class="btn btn-primary",_onclick="window.location='%s';"%URL(r=request,f='new',vars=request.vars)))
